...
//check http://www.doctort.org/adam/nerd-notes/x11-fake-keypress-event.html for keypress sending
...
void filter_and_threshold(struct ctx *ctx)
{
	...
	cvInRangeS(ctx->temp_image3,
		   cvScalar(0, 55, 50, 255),
		   cvScalar(65, 175, 255, 255),
           //cvScalar(0, 0, 0, 255),
           //cvScalar(200, 0, 1000, 255),
		   ctx->thr_image);
...
{
	int width = cvGetCaptureProperty(ctx->capture, CV_CAP_PROP_FRAME_WIDTH);
	int height = cvGetCaptureProperty(ctx->capture, CV_CAP_PROP_FRAME_HEIGHT);
	int i;
	...
	//P2->x = 60;
	//P2->y = 60;
 	cvRectangle(ctx->image, cvPoint(0, paddleY), cvPoint(20, paddleY+100), GREEN, 1, 8, 0);
	//goal top	
	cvRectangle(ctx->image, cvPoint(width, 0), cvPoint(width-40, 150), GREEN, 1, 8, 0);
	//goal bottom
	cvRectangle(ctx->image, cvPoint(width, height-150), cvPoint(width-40, height), GREEN, 1, 8, 0);
	
	//ball	
	cvRectangle(ctx->image, cvPoint(ballx, bally), cvPoint(ballx+ballsize, bally+ballsize), RED, 1, 8, 0);
	//ballmovement
	...
	if(ballx == 0 || ballx == width-20-ballsize || ballx == 20 && bally < paddleY+100 && bally >= paddleY || ballx == width-ballsize-20 && bally > 150 && bally < height-150){
		balldirx *= (-1);	
	}
	if(ballx == width-ballsize-20 && bally > 150 && bally < height-150){
		score += 1;
		printf("Scored!\n");
...
void detectMove(struct ctx *ctx){
	int width = cvGetCaptureProperty(ctx->capture, CV_CAP_PROP_FRAME_WIDTH);
	int height = cvGetCaptureProperty(ctx->capture, CV_CAP_PROP_FRAME_HEIGHT);
    if(ctx->lastLocY - ctx->hand_center.y > 10){
    ...
    else if(ctx->lastLocY - ctx->hand_center.y < -10){
        //printf("Was: %d Now: %d\n", ctx->lastLocY, ctx->hand_center.y);
        //printf("Moved: Down\n");
